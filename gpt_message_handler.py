...
def save_to_db(user_input, bot_response, user, message_id):
    now = datetime.now()
    query = db.insert(conversations).values(user_id=user.id, user_name=user.username, user_message=user_input, message_id=message_id, bot_response=bot_response, created_at=now)
